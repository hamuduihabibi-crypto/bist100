...
| **Performans Optimizasyonu** | `download_batch()` evreni **parçalı (chunked)** indirir: 278 hisse, `chunk_size=40`'lık alt gruplara bölünür; her parça kendi `timeout=20` değeriyle `yf.download` edilir ve sütun ekseninde tek MultiIndex çerçevede birleştirilir. Tek bir takılan/yavaş hisse tüm batch'i kilitlemez, bellek (RAM) yükü düşük tutulur (free tier 512MB). Ardından `ThreadPoolExecutor` (max_workers=4) ile paralel gösterge hesabı. Yerel ölçüm: 278 hisse ~11 sn (tek dev batch ~30 sn idi). Sonuç **1 saat bellekte cache'lenir** (`SCAN_CACHE_TTL=3600`) — free tier'da ilk tarama yine birkaç dakika sürebildiği için TTL tarama süresinin üzerinde tutulur (yoksa `/api/scan` sonsuza dek `202` döner). |
